...
def non_max_suppression(
    ...
    time_limit = 0.05 + 0.05 * bs  # seconds to quit after
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if (time.time() - t) > time_limit:
            ...
            print((f'WARNING ⚠️ NMS time limit {(time.time() - t):.3f}s exceeded'))
...
def nms(box_out, conf_thres=0.4, iou_thres=0.5):
    try:
        boxout = non_max_suppression(box_out, conf_thres=conf_thres, iou_thres=iou_thres, multi_label=True, classes=[0])
    except:
        boxout = non_max_suppression(box_out, conf_thres=conf_thres, iou_thres=iou_thres)
    return boxout

class YoloDet(object):

    current_file_path = os.path.dirname(os.path.realpath(__file__))
    YOLO_OM = os.path.join(current_file_path, "model/yolo.om")
    LABEL = os.path.join(current_file_path, "model/coco_names.txt")
    ...
    def __init__(
        ...
        self.labels_dict = self._get_labels_from_txt(label_path)
        self.cfg = {
    ...
    def inference(self, image):
        # 数据预处理
        img, scale_ratio, pad_size = self._preprocess_image(image, self.cfg, bgr2rgb=True)
        # 模型推理
    ...
    def _get_labels_from_txt(self, path):
        """从txt文件获取图片标签"""
        labels_dict = dict()
        with open(path) as f:
    ...
    def _preprocess_image(self, image, cfg, bgr2rgb=True):
        """图片预处理"""
