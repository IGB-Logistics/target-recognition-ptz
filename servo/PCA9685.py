...
class PCA9685(object):
    ...
    def set_pwm(self, channel, on, off):
        ...
        self.i2cBus.write_byte_data(self.address, self.LED0_ON_H + 4 * channel, on >> 8)
        self.i2cBus.write_byte_data(self.address, self.LED0_OFF_L + 4 * channel, off & 0xFF)
        self.i2cBus.write_byte_data(self.address, self.LED0_OFF_H + 4 * channel, off >> 8)

    def set_all_pwm(self, on, off):
        """Sets all PWM channels."""
        self.i2cBus.write_byte_data(self.address, self.ALL_LED_ON_L, on & 0xFF)
        self.i2cBus.write_byte_data(self.address, self.ALL_LED_ON_H, on >> 8)
        self.i2cBus.write_byte_data(self.address, self.ALL_LED_OFF_L, off & 0xFF)
        self.i2cBus.write_byte_data(self.address, self.ALL_LED_OFF_H, off >> 8)
    ...
    def set_pwm_freq(self, freq_hz):
        ...
        self.i2cBus.write_byte_data(self.address, self.MODE1, newmode) # go to sleep
        ...
        time.sleep(0.005)
        self.i2cBus.write_byte_data(self.address, self.MODE1, oldmode | 0x80)
